...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'approve': {
      ...
      const ids =
        target === 'all'
          ? listLeads(ctx.db, 'ready').map((l) => l.id)
          : [Number(target)];
      let n = 0;
      for (const id of ids) {
        try {
          approveLeadMessage(ctx, id, who);
          n++;
        } catch (e) {
    ...
    case 'status': {
      ...
      console.log(`\nSuppressed: ${listSuppression(ctx.db).length}`);
      console.log(`Recent events: ${listEvents(ctx.db).length} (showing last 8)`);
      for (const e of listEvents(ctx.db).slice(0, 8)) {
